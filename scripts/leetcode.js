...
const upload = (token, hook, code, problem, filename, sha, commitMsg, cb = undefined) => {
  const difficulty = document.querySelector('[class*=text-difficulty]')?.innerText
  const URL = `https://api.github.com/repos/${hook}/contents/LeetCode/${difficulty ? `${difficulty}/` : ''}${problem}/${filename}`;
...
async function getUpdatedData(token, hook, directory, filename) {
  const difficulty = document.querySelector('[class*=text-difficulty]')?.innerText
  const URL = `https://api.github.com/repos/${hook}/contents/LeetCode/${difficulty ? `${difficulty}/` : ''}${directory}/${filename}`;
